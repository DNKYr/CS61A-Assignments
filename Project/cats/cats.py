...
def about(subject):
    ...
    """
    assert all(
        [lower(x) == x for x in subject]
    ), "subjects should be lowercase."

    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"

    def is_subject_in_paragraph(paragraph):
    ...
    return is_subject_in_paragraph
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    correct_amount = 0
    incorrect_amount = max(0, typed_length - source_length)
    for i in range(source_length):
        if i >= typed_length:
            ...
            incorrect_amount += 1
    return correct_amount / (correct_amount + incorrect_amount) * 100
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        keys_for_cache = (typed, source)
        if keys_for_cache not in cache or limit > cache[keys_for_cache][1]:
            value = diff_function(typed, source, limit)
            cache[keys_for_cache] = (value, limit)
            return value
        return cache[keys_for_cache][0]
        # END PROBLEM EC
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed == source:
        ...
        return 0
        # END
    if limit <= 0 or limit < abs(len(typed) - len(source)):
        return limit + 1
    if not len(typed) or not len(source):
        return max(len(typed), len(source))
...
def fastest_words(words_and_times):
    ...
    check_words_and_times(
        words_and_times
    )  # verify that the input is properly formed
