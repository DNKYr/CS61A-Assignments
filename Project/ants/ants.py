...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if not place.ant.can_contain(self) and not self.can_contain(
                place.ant
            ):
    ...
    def double(self):
        ...
        if not hasattr(self, "doubleByQueen"):
            self.damage *= 2
            self.doubleByQueen = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place_checker.is_hive:
            if (
                place_checker.bees
                and distance >= self.lower_bound
                and distance <= self.upper_bound
            ):
                return place_checker.bees[
                    random.randint(0, len(place_checker.bees) - 1)
                ]
...
def random_bee(bees):
    ...
    assert isinstance(bees, list), (
        "random_bee's argument should be a list but was a %s"
        % type(bees).__name__
    )
    if bees:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            if gamestate.time % 2 == 0 or target.slowed <= 0:
                Bee.action(target, gamestate)
            target.slowed -= 1

        target.slowed = SlowThrower.slowed_down_time
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        target.scare(self.scared_distance)
        # END Problem EC 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while location and not location.is_hive:
            for bee in location.bees[:]:
                hit_insect[bee] = distance
            if location.ant and location.ant is not self:
                hit_insect[location.ant] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        return max(
            0, self.damage - (0.25 * distance) - (self.insects_shot * 0.0625)
        )
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # END Problem EC 3:with expression as target:
        pass
