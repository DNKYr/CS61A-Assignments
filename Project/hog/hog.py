...
def play(
    strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL
):
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    score, opponent_score, roll = 0, 0, -2
    while score < goal:
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    res, count, val = 0, 1, 0
    while count <= 10:
        test_v = make_averaged(roll_dice, times_called)(count, dice)
        if val < test_v:
            val = test_v
            res = count
        count += 1
...
def run_experiments():
    ...
    print(
        "always_roll(6) win rate:", average_win_rate(always_roll(6))
    )  # near 0.5
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if (
        sus_points(score + boar_brawl(score, opponent_score)) - score
        >= threshold
    ):
        return 0
    else:
