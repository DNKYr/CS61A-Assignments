...
def balanced(m):
    ...
    if is_planet(m):
        ...
        right_arm = right(m)
        return (
            total_mass(end(left_arm)) * length(left_arm)
            == total_mass(end(right_arm)) * length(right_arm)
...
def max_path_sum(t):
    ...
    """
    sum = label(t)
    sum += max([0]+[max_path_sum(branch) for branch in branches(t)])
    return sum
    """

    branches_sum = [max_path_sum(branch) for branch in branches(t)]
    return label(t) + (max(branches_sum) if branches_sum else 0)
